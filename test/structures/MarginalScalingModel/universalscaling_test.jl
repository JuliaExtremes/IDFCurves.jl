@testset "UniversalScaling" begin
    ...
    @testset "getdistribution(::UniversalScaling)" begin
        ...
        @test location(md) ≈ 99.79972127315776
        @test scale(md) ≈ 0.9979972127315776
        @test shape(md) ≈ 0.
    ...
    @testset "cdf(::UniversalScaling)" begin
        pd = UniversalScaling(1, 100, 1, 0, .8, 5, 10)
    ...
    #     @testset "initialize(::UniversalScaling)" begin
            
    #         init_vector = initialize(UniversalScaling, data, 1)
    ...
    #     fd = IDFCurves.fit_mle(UniversalScaling, data, 1, [20, 5, .04, .76, .07])
    #     H = IDFCurves.hessian(fd, data)

    #     @testset "fit_mle(::UniversalScaling, data, d₀, initialvalues)" begin

    #         @test [params(fd)...] ≈ [19.7911, 5.5938, 0.0405, 0.7609, 0.0681] rtol=.1
    #         fd2 = IDFCurves.fit_mle(UniversalScaling, data, 1, [20, 5, .0, .76, 1e-9])
    ...
    #     fd4 = IDFCurves.fit_mle(UniversalScaling, data_bug, 1)
    #     H2 = IDFCurves.hessian(fd4, data_bug)

    #     @testset "fit_mle(::UniversalScaling, data, d₀)" begin

    #         fd3 = IDFCurves.fit_mle(UniversalScaling, data, 1)
    #         @test [params(fd3)...] ≈ [params(fd)...] rtol=.01

    #     end

    #     @testset "hessian(::UniversalScaling, data)" begin
