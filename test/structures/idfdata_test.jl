@testset "IDFdata" begin
    ...
    @testset "_common_years" begin
        ...
        import IDFCurves._common_years

        tags = ["30min", "1h", "24h"]
        d1 = Dict("30min" => 0.5, "1h" => 1., "24h" => 24.)
        d2 = Dict("30min" => [years[1]], "1h" => years, "24h" => years)
        d3 = Dict("30min" => [y[1, 1]], "1h" => y[:, 2], "24h" => y[:, 3])

        s_incomplete = IDFdata(tags, d1, d2, d3)

        @test _common_years(s, gettag(s)) == getyear(s, "30min")
        @test _common_years(s_incomplete, gettag(s_incomplete)) == getyear(s_incomplete, "30min")
    end


    @testset "_validation_tag" begin

        import IDFCurves._validation_tag

        @test _validation_tag(s, "30min") == "30min"
        @test _validation_tag(s) == "30min"
        @test_throws ArgumentError _validation_tag(s, "nonextistant_tag")

    end

    @testset "_restrict_years" begin
        import IDFCurves._restrict_years
        new_data = _restrict_years(s, [2020])
        @test getyear(new_data, "30min") == [2020]
    end
