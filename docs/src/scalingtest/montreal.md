# Montréal (QC) precipitation IDF data
...
Load the IDF data recorded at Montréal–Pierre-Elliott-Trudeau International
Airport:
...
The analytical p-value is very small, providing strong evidence against the
...
The bootstrap p-value is also very small, again providing strong evidence
...
fig #hide
# draw(PDF("Mtl_cdf_simplescaling.pdf"), fig) #hide
```

## Testing the general scaling assumption

The general scaling model is fitted to all durations except the 5-minute
duration. The fitted model is then used to test the implied distribution of the
held-out 5-minute data.
...
The analytical p-value is very large, not providing sufficient evidence against the
general scaling assumption.

### Bootstrap calibration

The inter-duration dependence can be accounted for using a
year-wise bootstrap. The following example uses 999 bootstrap replicates:
...
The bootstrap p-value is also large, again not providing sufficient evidence
against the general scaling assumption.

### Visual comparison

As a visual diagnostic, compare the distribution predicted by the general
scaling model fitted without the 5-minute duration with the empirical
distribution of the held-out 5-minute observations.


```@example montreal
gs_model = T.fitted_model

distribution_5min =
    IDFCurves.getdistribution(gs_model, 5 / 60)
```
