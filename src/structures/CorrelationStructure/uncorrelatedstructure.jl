...
params_number(::UncorrelatedStructure) = 0
...
"""
    construct_model(::Type{<:UncorrelatedStructure, θ)
...
"""
    construct_model(::UncorrelatedStructure, θ)

Construct an UncorrelatedStructure from a set of transformed parameters θ in the real space.
"""
function construct_model(::UncorrelatedStructure, θ::AbstractVector{<:Any})
    @assert length(θ) == 0 "The parameter vector length must be 0 for an uncorrelated structure."
...
"""
    map_to_real_space(::UncorrelatedStructure, θ)

Map the parameter(s) from the UncorrelatedStructure parameter space to the real space.
"""
function map_to_real_space(::UncorrelatedStructure, θ::AbstractVector{<:Any})
    @assert length(θ) == 0 "The parameter vector length must be 0 for an uncorrelated structure."
